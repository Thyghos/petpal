...
private struct PetpalBackupEnvelope: Codable {
    ...
    var petProfileVaccines: [PetProfileVaccineDTO]?
    var petProfileMedications: [PetProfileMedicationDTO]?
    var petProfileAttachments: [PetProfilePetAttachmentDTO]?
}
...
private struct PetDTO: Codable {
    ...
    var careCardAttachmentImageData: Data?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case emergencyContactName, emergencyContactNumber, allergies
        case specialNotes, careCardAttachmentImageData
        case nextVetAppointmentDate
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        emergencyContactNumber = try c.decodeIfPresent(String.self, forKey: .emergencyContactNumber) ?? ""
        ...
        careCardAttachmentImageData = try c.decodeIfPresent(Data.self, forKey: .careCardAttachmentImageData)
        nextVetAppointmentDate = try c.decodeIfPresent(Date.self, forKey: .nextVetAppointmentDate)
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try c.encode(emergencyContactName, forKey: .emergencyContactName)
        try c.encode(emergencyContactNumber, forKey: .emergencyContactNumber)
        try c.encode(allergies, forKey: .allergies)
        try c.encode(specialNotes, forKey: .specialNotes)
        try c.encodeIfPresent(careCardAttachmentImageData, forKey: .careCardAttachmentImageData)
        try c.encodeIfPresent(nextVetAppointmentDate, forKey: .nextVetAppointmentDate)
    }
}
...
private struct PetProfileVaccineDTO: Codable {
    var id: UUID
    var petId: UUID
    var name: String
    ...
    init?(model: VaccineEntry) {
        guard let pid = model.pet?.id else { return nil }
        id = model.id
        petId = pid
        name = model.name
    ...
    func toModel(pet: Pet) -> VaccineEntry {
        VaccineEntry(id: id, name: name, dateAdministered: dateAdministered, dateExpires: dateExpires, pet: pet)
    }
}

private struct PetProfileMedicationDTO: Codable {
    var id: UUID
    var petId: UUID
    var name: String
    var amount: String
    ...
    func toModel(pet: Pet) -> MedicationEntry {
        MedicationEntry(id: id, name: name, amount: amount, frequency: frequency, pet: pet)
    }
}

private struct PetProfilePetAttachmentDTO: Codable {
    var id: UUID
    var petId: UUID
    var name: String
    var data: Data
    ...
    func toModel(pet: Pet) -> PetAttachment {
        PetAttachment(id: id, name: name, data: data, fileType: fileType, dateAdded: dateAdded, pet: pet)
    }
}
...
private struct PetRecordAttachmentDTO: Codable {
    ...
    enum CodingKeys: String, CodingKey {
        case id, parentRecordId, parentKind, fileData, contentKind, displayFileName, sourceNote, createdAt
    }

    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        id = try c.decode(UUID.self, forKey: .id)
        parentRecordId = try c.decode(UUID.self, forKey: .parentRecordId)
        parentKind = try c.decode(String.self, forKey: .parentKind)
        fileData = try c.decode(Data.self, forKey: .fileData)
        contentKind = try c.decode(String.self, forKey: .contentKind)
        displayFileName = try c.decodeIfPresent(String.self, forKey: .displayFileName) ?? ""
...
enum PetpalBackup {
    ...
    static func exportJSON(selection: BackupSelection, modelContext: ModelContext) throws -> Data {
        ...
        var envelope = PetpalBackupEnvelope(
            ...
            attachments: nil,
            petCertificates: nil,
            petProfileVaccines: nil,
            petProfileMedications: nil,
            petProfileAttachments: nil
        )

        if !petsOut.isEmpty {
            envelope.pets = petsOut.map(PetDTO.init(model:))
            if selection.includePets {
                let petIds = Set(petsOut.map(\.id))
                let vac = (try? modelContext.fetch(FetchDescriptor<VaccineEntry>())) ?? []
                envelope.petProfileVaccines = vac.compactMap { v -> PetProfileVaccineDTO? in
                    guard let dto = PetProfileVaccineDTO(model: v), petIds.contains(dto.petId) else { return nil }
                    return dto
                }
                let meds = (try? modelContext.fetch(FetchDescriptor<MedicationEntry>())) ?? []
                envelope.petProfileMedications = meds.compactMap { m -> PetProfileMedicationDTO? in
                    guard let dto = PetProfileMedicationDTO(model: m), petIds.contains(dto.petId) else { return nil }
                    return dto
                }
                let atts = (try? modelContext.fetch(FetchDescriptor<PetAttachment>())) ?? []
                envelope.petProfileAttachments = atts.compactMap { a -> PetProfilePetAttachmentDTO? in
                    guard let dto = PetProfilePetAttachmentDTO(model: a), petIds.contains(dto.petId) else { return nil }
                    return dto
                }
            }
        }
    ...
    private static func deleteAllSwiftData(modelContext: ModelContext) throws {
        ...
        for x in try modelContext.fetch(FetchDescriptor<VaccineEntry>()) {
            modelContext.delete(x)
        }
        for x in try modelContext.fetch(FetchDescriptor<MedicationEntry>()) {
            modelContext.delete(x)
        }
        for x in try modelContext.fetch(FetchDescriptor<PetAttachment>()) {
            modelContext.delete(x)
        }
    ...
    private static func insertAll(from env: PetpalBackupEnvelope, modelContext: ModelContext) throws {
        ...
        if let rows = env.petProfileVaccines {
            for dto in rows {
                if let pet = try fetchPet(id: dto.petId, modelContext: modelContext) {
                    modelContext.insert(dto.toModel(pet: pet))
                }
            }
        }
        if let rows = env.petProfileMedications {
            for dto in rows {
                if let pet = try fetchPet(id: dto.petId, modelContext: modelContext) {
                    modelContext.insert(dto.toModel(pet: pet))
                }
        ...
        if let rows = env.petProfileAttachments {
            for dto in rows {
                if let pet = try fetchPet(id: dto.petId, modelContext: modelContext) {
                    modelContext.insert(dto.toModel(pet: pet))
                }
    ...
    private static func merge(from env: PetpalBackupEnvelope, modelContext: ModelContext) throws {
        ...
        if let rows = env.petProfileVaccines {
            for dto in rows {
                if try fetchVaccineProfile(id: dto.id, modelContext: modelContext) == nil {
                    if let pet = try fetchPet(id: dto.petId, modelContext: modelContext) {
                        modelContext.insert(dto.toModel(pet: pet))
                    }
                }
        ...
        if let rows = env.petProfileMedications {
            for dto in rows {
                if try fetchMedicationProfile(id: dto.id, modelContext: modelContext) == nil {
                    if let pet = try fetchPet(id: dto.petId, modelContext: modelContext) {
                        modelContext.insert(dto.toModel(pet: pet))
                    }
                }
        ...
        if let rows = env.petProfileAttachments {
            for dto in rows {
                if try fetchPetAttachmentProfile(id: dto.id, modelContext: modelContext) == nil {
                    if let pet = try fetchPet(id: dto.petId, modelContext: modelContext) {
                        modelContext.insert(dto.toModel(pet: pet))
                    }
                }
    ...
    private static func fetchVaccineProfile(id: UUID, modelContext: ModelContext) throws -> VaccineEntry? {
        let idv = id
        var d = FetchDescriptor<VaccineEntry>(predicate: #Predicate { $0.id == idv })
        d.fetchLimit = 1
        return try modelContext.fetch(d).first
    }

    private static func fetchMedicationProfile(id: UUID, modelContext: ModelContext) throws -> MedicationEntry? {
        let idv = id
        var d = FetchDescriptor<MedicationEntry>(predicate: #Predicate { $0.id == idv })
        d.fetchLimit = 1
        return try modelContext.fetch(d).first
    }

    private static func fetchPetAttachmentProfile(id: UUID, modelContext: ModelContext) throws -> PetAttachment? {
        let idv = id
        var d = FetchDescriptor<PetAttachment>(predicate: #Predicate { $0.id == idv })
        d.fetchLimit = 1
        return try modelContext.fetch(d).first
    }
...
private extension PetDTO {
    init(model: Pet) {
        ...
        careCardAttachmentImageData = model.careCardAttachmentImageData
        nextVetAppointmentDate = model.nextVetAppointmentDate
    }
    ...
    func toModel() -> Pet {
        Pet(
            ...
            careCardAttachmentImageData: careCardAttachmentImageData,
...
private extension Pet {
    func apply(from dto: PetDTO) {
        ...
        careCardAttachmentImageData = dto.careCardAttachmentImageData
        nextVetAppointmentDate = dto.nextVetAppointmentDate
    }
}
...
private extension PetRecordAttachment {
    func apply(from dto: PetRecordAttachmentDTO) throws {
        ...
        displayFileName = dto.displayFileName
