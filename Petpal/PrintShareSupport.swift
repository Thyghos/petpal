...
struct PrintShareHelper {
    ...
    /// Single-page PDF from a UIImage (Care Card export). Filename should include `.pdf`.
    @MainActor
    static func writeSinglePagePDF(from image: UIImage, fileName: String) -> URL? {
        let safeName = fileName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Export.pdf" : fileName
