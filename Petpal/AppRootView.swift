    ...
    var body: some View {
        MainTabView()
            ...
            .task {
                ...
                await MilestoneCheckService.shared.runStartupChecks(modelContext: modelContext)
                let petsForYIR = (try? modelContext.fetch(FetchDescriptor<Pet>())) ?? []
                let sortedYIR = petsForYIR.sorted { $0.dateAdded < $1.dateAdded }
                await YearInReviewNotificationService.scheduleYearInReviewNotifications(pets: sortedYIR)
                #endif
            }
            #if os(iOS)
            ...
            .onReceive(NotificationCenter.default.publisher(for: .petpalSwiftUIForegroundRefresh)) { _ in
                refreshID = UUID()
            }
            #endif
