...
/// Trailing navigation bar control: circular chip, gradient SF Symbol (matches Pet tab gear / add).
struct NavBarCircleIconButton: View {
    ...
    let adaptiveShadow: Color
    /// Square chip size (default 44 for nav bar; use ~28 for compact card overlays).
    var chipSize: CGFloat = 44
    let action: () -> Void

    private var iconFont: Font {
        if chipSize <= 32 {
            return .system(size: max(11, chipSize * 0.39), weight: .semibold)
        }
        return .title3
    }

    private var chipShadowRadius: CGFloat { chipSize <= 32 ? 5 : 10 }
    private var chipShadowY: CGFloat { chipSize <= 32 ? 2 : 4 }
            ...
            Image(systemName: systemImage)
                ...
                .background(
                    Circle()
                        ...
                        .shadow(
                            color: adaptiveShadow.opacity(0.9),
                            radius: chipShadowRadius,
                            x: 0,
                            y: chipShadowY
                        )
                )
...
// MARK: - Export card hex colors (ImageRenderer; fixed appearance, not dynamic)

extension Color {
    /// Six-digit RGB hex, e.g. `"E8622A"` or `"#E8622A"`.
    init(hex: String) {
...
/// Shared footer for dark export cards (milestone, recap, year-in-review slides).
@ViewBuilder
func exportBrandingFooterDark(centerY: CGFloat, layoutScale: CGFloat, displayWidth: CGFloat) -> some View {
    PetpalBrandMark(size: displayWidth * 0.045, style: .darkBackground)
        ...
        .position(x: displayWidth / 2, y: centerY)
}
