...
// Optional developer-provided insurance tips / external links (same for every pet).
...
struct InsuranceCuratedItem: Identifiable, Hashable {
    ...
    let title: String
    let detail: String?
    /// Set when you add an external URL; opens in Safari / in-app browser.
    let url: URL?
...
enum InsuranceCuratedContent {
    ...
    static let items: [InsuranceCuratedItem] = [
        // Intentionally empty until you add optional curated rows.
    ]
}

// MARK: - Affiliate disclosure (Pet Deals)

/// Footer on **Pet Deals** when outbound links may include affiliate or partner programs.
