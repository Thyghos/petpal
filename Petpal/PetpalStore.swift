...
//     com.thyghos.petpalapp.vetai.plus.monthly — $3.99/month  (75 replies)
//     com.thyghos.petpalapp.vetai.pro.monthly  — $9.99/month (250 replies)
//   Retired subscription IDs (burned; do not reference in code):
//     com.thyghos.petpalapp.aivet.plus, com.thyghos.petpalapp.aivet.pro,
//     com.thyghos.petpalapp.aivet.sub.plus, com.thyghos.petpalapp.aivet.sub.pro
//   Consumable:
...
final class PetpalStore: ObservableObject {
    ...
    static let plusProductID = "com.thyghos.petpalapp.vetai.plus.monthly"
    static let proProductID  = "com.thyghos.petpalapp.vetai.pro.monthly"
    ...
    func loadProducts() async {
        print("[PetpalStore] loadProducts() started")
        let ids = Array(Self.subscriptionIDs).sorted()
        print("[PetpalStore] loadProducts() requesting product IDs: \(ids)")
        isLoading = true
        defer { isLoading = false }
        do {
            let products = try await Product.products(for: Self.subscriptionIDs)
            print("[PetpalStore] loadProducts() returned \(products.count) product(s)")
            subscriptions = products.sorted { $0.price < $1.price }
        } catch {
            print("[PetpalStore] loadProducts() error: \(String(reflecting: error))")
    ...
    func purchase(_ product: Product) async -> Bool {
        print("[PetpalStore] purchase() called for product id=\(product.id) displayName=\(product.displayName)")
        purchaseError = nil
        do {
