# Terms of Service for Petpal

**Last Updated: March 31, 2026**

## Agreement to Terms

By accessing or using the Petpal mobile application ("App"), you agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, do not use the App.

## Description of Service

Petpal is a pet health tracking and information application that allows you to:
- Store and organize your pet's health information (profiles, health history, vet documents, certificates, weight, insurance records you enter)
- Set reminders for medications, appointments, and care tasks
- Access general pet care information through an optional AI assistant (Vet AI)
- Customize which features appear on your home screen
- Generate emergency QR codes for pet information
- Export or import selected data via backup (when you use that feature)
- Open optional **Pet Picks** and **Insurance** links to third-party sites (some may be affiliate or partner links—see the Privacy Policy)

**Not included in the current app:** map-based travel or place search, a standalone “Shop” tile, or continuous location tracking for discovery features.

## Medical Disclaimer

### NOT MEDICAL ADVICE

**IMPORTANT**: Petpal is NOT a substitute for professional veterinary medical care.

- **No Diagnosis**: This App does not diagnose illnesses or medical conditions
- **No Treatment**: This App does not provide medical treatment plans
- **No Prescriptions**: This App cannot prescribe medications
- **No Emergency Care**: This App is NOT for medical emergencies

### AI Assistant Limitations

The AI assistant feature:
- Provides general pet care information only
- Uses artificial intelligence which may produce errors
- Cannot replace consultation with a licensed veterinarian
- Should not be relied upon for medical decisions

### Emergency Situations

**IN CASE OF EMERGENCY**: Contact your veterinarian or emergency animal hospital immediately. Do not rely on this App for emergency medical situations including but not limited to:
- Difficulty breathing
- Severe bleeding
- Seizures
- Collapse or unconsciousness
- Suspected poisoning
- Severe injury
- Any life-threatening condition

### Professional Consultation Required

ALWAYS seek the advice of your veterinarian or other qualified animal health provider with any questions about your pet's medical condition. Never disregard professional veterinary advice or delay seeking it because of information provided by this App.

## User Responsibilities

You agree to:
- Provide accurate information about your pet
- Use the App for lawful purposes only
- Not rely on the App for medical decisions
- Consult a veterinarian for health concerns
- Keep your device and App secure
- Not attempt to hack, reverse engineer, or misuse the App

## Intellectual Property

- The Petpal App and all content are owned by Emilio Alecci *(update if you publish under a company or LLC)*
- You may not copy, modify, distribute, or resell any part of the App
- Your pet data and photos remain your property
- We do not claim ownership of your content

## Data and Privacy

Your use of the App is governed by our Privacy Policy. Key points:
- Your data is stored on your device (and optionally syncs via **your** iCloud / CloudKit if you use that)
- We do not sell your personal information
- Optional AI (Vet AI) queries are processed by third-party providers you enable or that the app supports
- **Petpal does not collect or use your location** for features in this version
- Affiliate or partner links open in your browser; those sites have their own privacy practices

See our full Privacy Policy at: [privacy policy URL]

## Limitation of Liability

TO THE FULLEST EXTENT PERMITTED BY LAW:

**WE ARE NOT LIABLE FOR**:
- Any decisions made based on App information
- Errors or omissions in AI-generated content
- Missed reminders or notifications
- Lost or corrupted data
- Any harm to your pet
- Damages arising from App use or inability to use

**YOU USE THIS APP AT YOUR OWN RISK**.

The App is provided "AS IS" without warranties of any kind, express or implied.

## Disclaimer of Warranties

We do not warrant that:
- The App will be error-free or uninterrupted
- AI responses will be accurate or complete
- Reminders will always function correctly
- Data will never be lost
- The App will meet your specific needs

## Indemnification

You agree to indemnify and hold harmless Emilio Alecci *(or your registered business name, if different)* from any claims, damages, or expenses arising from:
- Your use of the App
- Your violation of these Terms
- Your violation of any rights of others
- Decisions made based on App information

## Third-Party Services

Depending on how you use the App, third parties may include:
- **Apple** (iCloud/CloudKit sync, StoreKit for purchases, platform services)
- **AI service providers** (optional Vet AI)
- **Optional proxy or relay** services if configured for AI requests
- **Hosting** for an optional static emergency page (if you set one up)
- **Websites you choose to open** from Pet Picks, Insurance, or other links (each site has its own terms)

Apple may provide **aggregated** crash or App Store analytics to the developer per Apple’s policies. This version of the App does **not** bundle third-party advertising or cross-app tracking SDKs.

Each third-party service has its own terms and conditions. We are not responsible for third-party services.

## Subscription and Payments

**Optional in-app purchases** (for example subscriptions related to Vet AI features and optional tips) are processed by **Apple** through StoreKit. Pricing and renewal terms appear in the App Store at purchase time. Purchases are subject to [Apple’s standard App Store terms](https://www.apple.com/legal/internet-services/itunes/).

## Termination

We reserve the right to:
- Terminate or suspend access to the App
- Modify or discontinue features
- Update these Terms at any time

You may stop using the App at any time by deleting it from your device.

## Changes to Terms

We may update these Terms periodically. Continued use of the App after changes constitutes acceptance of new Terms. We will notify you of significant changes.

## Age Requirement

You must be at least 13 years old (or legal age in your jurisdiction) to use this App. The App is intended for pet owners of legal age.

## Geographic Restrictions

- The App is available where permitted by law
- Some features may not be available in all regions
- You are responsible for compliance with local laws

## Governing Law

These Terms are governed by the laws of the **United States** and the **State of Florida**, without regard to conflict of law principles.

## Dispute Resolution

Any disputes arising from these Terms or use of the App shall be resolved through:
1. Good faith negotiation
2. Binding arbitration (if required)
3. State and federal courts located in **Florida, United States** (for non-arbitrable disputes)

## Severability

If any provision of these Terms is found unenforceable, the remaining provisions remain in full effect.

## Entire Agreement

These Terms, together with the Privacy Policy, constitute the entire agreement between you and the operator of Petpal regarding the App.

## Contact Us

Questions about these Terms?

**Operator**: Emilio Alecci (Florida, United States)  
**Email (legal & support)**: ealecci@gmail.com  
**Website**: https://thyghos.github.io/petpal-privacy/ *(update if you host these pages elsewhere)*

## Acknowledgment

BY USING PETPAL, YOU ACKNOWLEDGE THAT:
- You have read and understood these Terms
- You have read and understood the medical disclaimers
- You agree to use the App for informational purposes only
- You will consult a veterinarian for medical advice
- You will not use the App for emergencies
- You accept all risks associated with App use

---

**Effective Date**: March 31, 2026

**Important Reminder**: Petpal is a tool to help you organize your pet's information and access general pet care tips. It is NOT a replacement for professional veterinary care. Always consult your veterinarian for medical advice, diagnosis, and treatment.

🐾 Thank you for using Petpal responsibly!
